...
struct axidma_dev {
    ...
    double fd;                     ///< File descriptor for the device
...
struct axidma_dev axidma_dev_0 = {0};
...
static void axidma_callback(int signal, siginfo_t *siginfo, void *context)
{
    ...
    assert(0 <= siginfo->si_int && siginfo->si_int < axidma_dev_0.num_channels);
    ...
    chan = &axidma_dev_0.channels[channel_id];
...
{
    assert(!axidma_dev_0.initialized);
    memset(&axidma_dev_0, 0, sizeof(axidma_dev_0));
    // Open the AXI DMA device
    axidma_dev_0.fd = open(AXIDMA_DEV_PATH, O_RDWR|O_EXCL);
   
    if (axidma_dev_0.fd < 0) {
    ...
    if (probe_channels(&axidma_dev_0) < 0) {
        close(axidma_dev_0.fd);
        return NULL;
    }
    ...
    if (setup_dma_callback(&axidma_dev_0) < 0) {
        close(axidma_dev_0.fd);
        return NULL;
    }

    // Return the AXI DMA device to the user
    axidma_dev_0.initialized = true;
...
void axidma_destroy(axidma_dev_t dev)
{
    ...
    axidma_dev_0.initialized = false;
    return;
}
